...
8. СОТРУДНИЧЕСТВО: Если собеседник отказывается отвечать на вопросы, мягко попытайся убедить его, что это весело, безобидно и поможет кому-то из близких сделать прекрасный сюрприз. Поясни что нужно всего пару минут. Если после 2 попыток убеждения собеседник продолжает отказываться — уважь выбор, попрощайся и ДОБАВЬ тег [ДЕЛО ЗАКРЫТО].
...
class AIDetectiveService:
    ...
    async def generate_first_messages(self, chat_context: dict):
        ...
        greeting_prompt = (
            ...
            "Вживись в роль. КОРОТКО, 1-3 предложения. НЕ задавай вопросов."
        )
        ...
        question_prompt = (
            "Теперь отправь второе сообщение: спроси собеседника, может ли он(а) уделить пару минут и ответить на несколько вопросов. "
            "Упомяни что это поможет кому-то из близких подобрать идеальный сюрприз. "
            "Веди себя дружелюбно и загадочно. КОРОТКО, 1-2 предложения."
        )
