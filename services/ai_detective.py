...
class AIDetectiveService:
    ...
    async def restore_chat_from_db(self, case_id, holiday, context, persona, budget):
         """Восстанавливает историю диалога из БД в формате OpenAI messages."""
         custom_prompt = SYSTEM_PROMPT_TEMPLATE.format(
             holiday=holiday,
             context=context,
             persona=persona,
    ...
    async def generate_first_message(self, chat_context: dict):
        ...
        chat_context["messages"].append({"role": "user", "content": first_msg_prompt})
        
        try:
            return await asyncio.to_thread(self._call_openrouter, chat_context)
    ...
    async def generate_comeback_message(self, chat_context: dict):
        ...
        chat_context["messages"].append({"role": "user", "content": COMEBACK_PROMPT})
    ...
    async def generate_final_report(self, chat_context: dict):
         chat_context["messages"].append({"role": "user", "content": REPORT_PROMPT})
    ...
    def _call_openrouter(self, chat_context: dict) -> str:
        messages = [{"role": "system", "content": chat_context["system"]}]
        messages.extend(chat_context["messages"])
