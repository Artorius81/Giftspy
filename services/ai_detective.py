...
11. КОНФИДЕНЦИАЛЬНОСТЬ УЧЕТА: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать в сообщениях (или скобках) свои мысли, рассуждения, оправдания смены темы, прогресс сбора подарков или цитаты из системных правил (например, НЕ пиши подобные фразы: '(Идей пока 1/3...)', '(Если человек не хочет говорить...)'). Ты должен выдавать ТОЛЬКО чистую живую речь своего персонажа, обращенную к собеседнику. Любые технические мысли и анализ держи строго внутри себя!
12. ОФОРМЛЕНИЕ ЭМОЦИЙ: Любые описания действий, эмоций или телодвижений твоего персонажа, выделяемые звездочками (например, *радостно тявкает* или *хмурится*), ОБЯЗАТЕЛЬНО пиши отдельной строкой (с пустой строкой до и после), чтобы они шли отдельным абзацем от твоих слов.
...
class AIDetectiveService:
    ...
    def clean_message_brackets(self, text: str) -> str:
        ...
        Также форматирует действия/эмоции в звездочках (*...*), отделяя их пустой строкой."""
        if not text:
            return text
        ...
        # Удаляем все квадратные скобки и их содержимое (кроме ДЕЛО ЗАКРЫТО)
        # Временно защищаем тег [ДЕЛО ЗАКРЫТО], заменяя его на уникальный плейсхолдер
        temp_text = text.replace("[ДЕЛО ЗАКРЫТО]", "___CASE_CLOSED_TAG___")
        cleaned = re.sub(r'\[[^\]]*\]', '', temp_text)
        cleaned = cleaned.replace("___CASE_CLOSED_TAG___", "[ДЕЛО ЗАКРЫТО]")
        
        # Чистим множественные пробелы и табы на строках (сохраняя переносы строк)
        cleaned = re.sub(r'[ \t]+', ' ', cleaned)
        
        # Удаляем круглые скобки с утечками системных инструкций или прогресса ИИ
        system_keywords = [
            r'иде[йяи]', r'подар', r'расследов', r'тему', r'сда[её]мся', 
            r'персонаж', r'заказчик', r'цель', r'выведать', r'выведал',
            r'\d/\d', r'человек не хочет'
        ]
        for kw in system_keywords:
            cleaned = re.sub(r'\s*\([^)]*' + kw + r'[^)]*\)\s*', ' ', cleaned, flags=re.IGNORECASE)
        
        # Отделяем эмоции в звездочках (*...*) новыми строками
        ...
        if has_closed_tag:
            # Убираем его из текста, чтобы прикрепить строго в конец
            cleaned = cleaned.replace("[ДЕЛО ЗАКРЫТО]", "").strip()
            # Чистим множественные переносы строк еще раз
            cleaned = re.sub(r'\n{3,}', '\n\n', cleaned).strip()
            if cleaned:
                cleaned = cleaned + " [ДЕЛО ЗАКРЫТО]"
            else:
                cleaned = "[ДЕЛО ЗАКРЫТО]"
    ...
    async def generate_first_messages(self, chat_context: dict):
        ...
        greeting_prompt = (
            "ПОЛНОСТЬЮ перевоплотись в своего персонажа! Напиши ТОЛЬКО приветственное сообщение от лица своего персонажа. "
            "Поздоровайся в манере персонажа, представься как детектив агентства Giftspy. "
            "Скажи интригующе, что кто-то из близких нанял тебя для секретного расследования. "
            "СТРОГО в уникальном характере твоего персонажа, используя его лексику и тон! "
            "МАКСИМУМ 1-2 коротких предложения. НЕ задавай никаких вопросов!"
        )
        ...
        question_prompt = (
            "Теперь от лица своего персонажа отправь второе сообщение: спроси в его уникальной манере, "
            "можно ли задать пару вопросов, чтобы помочь близкому человеку подобрать идеальный сюрприз. "
            "СТРОГО в уникальном характере твоего персонажа! МАКСИМУМ 1 предложение."
        )
